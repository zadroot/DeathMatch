...
public Event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (attacker && attacker != client)
	{
		if (g_ConVars[ConVar_Mode][Value] && GetClientTeam(client) == GetClientTeam(attacker))
		{
			...
			{
				LogError("Error: Unable to find offset: \"m_iFrags\"!");
			}
		...
		if (g_ConVars[ConVar_ShowHP][Value])
		{
			...
			{
				PrintToChat(client, "[DM] %t", "Health Remaining", attackerHealth);
			}
			else
			{
...
{
	return g_ConVars[ConVar_Mode][Value] ? Plugin_Handled : Plugin_Continue;
}
...
{
	// Remove map objectives from the HUD
...
{
	TR_TraceHull(vecOrigin, vecOrigin, Float:{ -24.0, -24.0, 0.0 }, Float:{ 24.0, 24.0, 82.0 }, MASK_PLAYERSOLID);
...
public Action:Timer_Respawn(Handle:timer, any:client)
{
	...
	{
		// If FFA mode is active, switch the player to a random team
		if (g_ConVars[ConVar_Mode][Value])
		{
			ChangeClientTeam(client, GetClientTeam(client) == Team_Allies ? Team_Axis : Team_Allies);
		}
